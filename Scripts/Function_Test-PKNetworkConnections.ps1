...
.EXAMPLE
    PS C:\> Test-PKNetworkConnection server222 -Verbose
        ...
        ScriptName            Test-PKNetworkConnection                        
        ScriptVersion         1.0.0                                    
        PipelineInput         False                                    
...
.EXAMPLE
    PS C:\> Get-VM ops*  | Test-PKNetworkConnection | Format-Table -AutoSize
...
Begin {
    ...
    $PipelineInput = (-not $PSBoundParameters.ContainsKey("ComputerName")) -and (-not $TName)
    ...
    function LookupDNS {
        ...
        If ($Target -as [ipaddress]) {
            Try {    
                If ($Lookup = [Net.Dns]::GetHostByAddress($Target)) {
                    Write-Output $Lookup
                }
            }Catch {}
        }
        Else {
            Try {    
                If ($Lookup = [Net.Dns]::GetHostEntry($Target)) {
                    Write-Output $Lookup
                }
            }Catch {}
        }
...
process{
    ...
    Foreach ($T in $Target){
	    ...
	    Try {
            ...
            If ($DNSEntity = LookupDNS -Target $T) {
                    
                $Output.DNSLookup = $DNSEntity.hostname

                If (-not $T -as [ipaddress]) {
        ...
        # If we have IP addresses we'll continue
        If ($IPs.Count -gt 0) {
            $Continue = $True
        }
        Else {
            $Msg = "No IP address information found for $T"
            $Host.UI.WriteErrorLine("ERROR: $Msg")
            $Results += $Output
        }
            ...
            foreach ($IP in $IPs) {
                ...
                If ($Selection -contains $Label) {                    
                    ...
                    Try {	
				        ...
				        $w.path = "\\$T\root\cimv2:Win32_ComputerSystem.Name='$T'"
