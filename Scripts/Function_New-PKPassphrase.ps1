...
<#
.SYNOPSIS
    Uses REST API calls to generate one or more passphrases from English or Lorem Ipsum word lists

.DESCRIPTION
    Calls external APIs to fetch English or Lorem Ipsum word lists and combines them with optional integers
    Configurable passphrase count, word count, separator character, integer count, and minimum length
    Available output formats are string (for testing only) or secure string (recommended for production)
...
.NOTES
    ...
    History :
        ...
        ** PLEASE KEEP $VERSION UPDATED IN PROCESS BLOCK **
        ...
        v02.00 - 2024-04-26 - Learned stuff, fixed stuff, added stuff
        v03.00 - 2025-03-21 - Replaced loripsum.net api with https://fakeapi.net/lorem
        v03.01 - 2026-06-17 - Cosmetic updates; converted version format to major.minor
...
Begin {
    ...
    Write-Verbose "[BEGIN: $ScriptName] Create new passphrases ($Dictionary dictionary)"
    $Msg = "Passphrases will consist of $NumWords $Dictionary words and $NumIntegers integer(s), separated by $($Separator.tolower()) characters, returning in $ReturnAs format"
    Write-Verbose $Msg
    If ($ReturnAs -eq "String") {
        $Msg = "Output will be returned as an insecure string (for production use, consider using -ReturnAs SecureString instead!)"
        Write-Warning $Msg
    }
}
Process {
